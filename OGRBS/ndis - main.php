...
if(isset($_REQUEST['dname']))
{
	...
	else
	{
		...
		{			
			...
			$ext = ".".$info['extension'];
			$target_file="distributor/proof/".$dmno.$ext;
			move_uploaded_file($_FILES["id_img"]["tmp_name"], $target_file);
			
			// For data insert
			$q = "INSERT INTO `distributor_detail`(`pwd`, `name`, `address`, `city`, `pin`, `m_no`, `e_id`, `proof`) 
					VALUES ('$dcpass', '$dname', '$dadd', '$dcity', $dpin, $dmno, '$demail', '$target_file');";
