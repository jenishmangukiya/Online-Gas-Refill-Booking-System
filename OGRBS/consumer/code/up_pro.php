...
{	
	...
	{
		//mobile no update
		$result=mysqli_query($conn,"select m_no from consumer_detail where m_no=$data AND cid!=$cid") or mysqli_error($conn);
		...
		{
			echo 'True';
			mysqli_query($conn,"update consumer_detail set m_no=$data where cid=$cid");
		}
	}
	else if($_POST['sub']=='e')
	{
		//email update	
		$result=mysqli_query($conn,"select e_id from consumer_detail where e_id='$data' AND cid!=$cid") or mysqli_error($conn);
		...
		{
			echo 'True';
			mysqli_query($conn,"update consumer_detail set e_id='$data' where cid=$cid");
		}
	}
	else if($_POST['sub']=='p')
	{
		//password update
		echo 'True';
		mysqli_query($conn,"update consumer_detail set pwd='$data' where cid=$cid");	
	}
}
else
{
